...
class saarelma_connor_nondim(saarelma_connor):
    ...
    def _set_nondim_scales(self, verbose=False):
        ...
        Convention (see App.~A.8):
            ...
            L  = |x_inner|             (m)
            n0 = ne_x0                 (m^-3)
            T0 = (T_e(0)+T_i(0)) * eV  (J)            -- separatrix value
            S0 = S_i(0)                (m^3/s)        -- separatrix value
            tau    = 1 / (n0 * S0)
            [V]_0  = L / tau
            [D]_0  = L^2 / tau

        Both ``self.x_inner`` and the separatrix index ``-1`` of the
        pressure / Te / S_i arrays must already be defined.  In
        :meth:`solve_coupled` this is guaranteed by the prior call to
        :meth:`_ensure_firedrake_coefficient_grids` and to
        :meth:`find_inner_boundary` (or the user-supplied
        ``psi_N_inner_boundary``).
        """
    ...
    def _interp_si_to_hat(self, hat_x_dofs, si_array_on_xinit):
        ...
        return np.interp(x_si, self.x_init, np.asarray(si_array_on_xinit))
    ...
    def _ensure_firedrake_discretization_nondim(self, mesh_n, fe_degree, force=False):
        ...
        hat_g_arr = self._interp_si_to_hat(hat_x_dofs, self.gradr2_fsa)
    ...
    def calc_pressure_quantities_nondim(self, hat_n_e, average_alpha_pedestal=True):
        ...
        # Connor-Hastie alpha = alpha_nodp * dp/dx (SI), as in solver.py.
        _pres = n_e_sorted * T_e_eV * _EV2J            # Pa
    ...
    def solve_coupled(self,
        ...
        # Rescale BC values into hat-units (App. A.8 Eq. (eq:hat-flux-A8) etc.)
        hat_ne_x0      = 1.0
        hat_ne_inner   = ne_inner_val   / self._n0_nd
        ...
        T_e_J = self.T_e_pres * _EV2J
        T_i_J = self.T_i_pres * _EV2J
        hat_T_arr = (
